...
const Futures = () => {
  ...
  const currencySectionData = {
    ...
    paragraphs: [
      "Currencies are always traded in pairs. The first currency in the pair is called the base currency and the second the quote currency. Futures traders buy one currency in the pair and sell the other at the same time, depending on how they think their values will change in relation to each other.",
      "Currency values can be affected by a number of factors that impact the strength of a nation's economy, including inflation, interest rates and political stability to name a few. Typically, Futures traders see the greatest volatility before and after key economic or political events as they speculate how this could affect the markets.",
      "Currency pairs are split into three categories. Major currency pairs are the most traded pairs in the Futures industry. They usually include the US dollar as one half of the pair, like the EUR/USD and the USD/JPY. Other commonly traded major pairs include EUR/JPY and the EUR/GBP.",
    ],
    image: globeImg,
  };
  ...
  const videoData = {
    ...
    description:
      "Futures (or FX) means Foreign Exchange, the place where currencies are traded. To learn more, check out our complete guide to",
    linkText: "Futures trading for beginners.",
  ...
  const faqItems = [
    {
      question:
        "How much do I need to start trading Futures with RightTrade Capital?",
      answer:
    ...
    {
      question: "Which account type is best for trading Futures?",
      answer:
    ...
    {
      question: "Is Futures trading safe?",
      answer:
        "Futures trading involves risks. It is important to trade with a regulated broker and have a solid risk management plan in place.",
    },
    {
  ...
  return (
    <div className="w-full">
      ...
      {/*  2 Section */}
      <ForexTradeSection
        title={FuturesContent.title}
        description={FuturesContent.description}
        benefits={FuturesContent.benefits}
      />
      {/*  3 Section */}
      <WhyTradeForexSection
        title="Why trade Futures currency pairs with RightTrade Capital?"
